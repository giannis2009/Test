...
const { db, saveSettings, DEFAULT_SETTINGS } = require('../db');
const {
...
router.get('/', (req, res) => {
  ...
  const topViewed = db.prepare("SELECT * FROM cars WHERE status != 'draft' ORDER BY views DESC LIMIT 5").all();
  ...
  res.render('admin/dashboard', { title: 'Πίνακας ελέγχου', stats, topViewed, latestInquiries, noPhotos });
});
...
router.get('/cars', (req, res) => {
  ...
  res.render('admin/cars', { title: 'Αγγελίες', ...result });
});
...
router.get('/inquiries', (req, res) => {
  const inquiries = db.prepare(
    `SELECT q.*, c.make, c.model, c.version, c.id AS cid FROM inquiries q
     LEFT JOIN cars c ON c.id = q.car_id ORDER BY q.is_read ASC, q.created_at DESC LIMIT 500`,
  ).all();
  res.render('admin/inquiries', { title: 'Μηνύματα', inquiries });
});
